...
function openMap() {
	...
	var	lat =document.getElementById('user_latitude').value;
	var	lng = document.getElementById('user_longitude').value;
	if(lat !="" && lng !=""){
		userlatitude = lat;
		userlongitude = lng;
	}
	...
	$.ajax({
		...
		success : function(result){ 
			...
			for (var i = 0; i < result.result.length; i++) {
				positions.push({
							content : '<div>' + result.result[i][1].acc_name
									+ '</div>',
...
function openaddr() {
	...
	document.getElementById('map-modal-container').classList
	.add('map-modal-container');
	document.getElementById('addrmodal-container').classList
	.add('addr-modal-wrap');
	
	var mapContainer = document.getElementById('addrmodal-container'), // 지도를 표시할 div 
    mapOption = {
...
function addr_search(){
	...
	var add_search = document.getElementById('addr-search-form').value;
// 주소로 좌표를 검색합니다
...
function addr_change(){
	...
	document.getElementById('user_latitude').value = LatLng2.getLat();
	document.getElementById('user_longitude').value = LatLng2.getLng();
	distance_();
